...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (self.data.numpy()[idx], self.labels.numpy()[idx])
...
def LoadBatch(batch_no, device):
    ...
    img_batch, label_batch = np.expand_dims(np.array([_ for _ in batch[:, 0]]), axis = 1), np.array([_ for _ in batch[:, 1]])
    return torch.tensor(img_batch, dtype = torch.float32, device = device), torch.tensor(label_batch, dtype = torch.long, device = device)
    ...
    for epoch in range(epochs):
        ...
        for jdx, file in enumerate(Path(str(Path(__file__).parent.parent) + '\\training_batches\\').glob('**/*.npy')):
            ...
            for idx, data in enumerate(trainloader, 0):
                ...
                if idx % 4 == 3:
                    print("Epoch: {:3d} | Ieration : {:3d} | Loss : {:3.3f}".format(epoch + 1, sample_counter + idx + 1, running_loss))
                    running_loss    =   0
...
def main():
    ...
    train(conv_network, params, 5, DEVICE)
