...
def get_images(count):
    testFolderNormal    =   str(Path(__file__).parent.parent.parent) + "\\scaled_dataset\\test\\NORMAL\\"
    testFolderPneumonia =   str(Path(__file__).parent.parent.parent) + "\\scaled_dataset\\test\\PNEUMONIA\\"
    ...
    return test_batch, samples

def convert_labels(int_labels):
    label_dict  =   {0 : 'Healthy', 1 : 'Pneumonia'}
    str_labels  =   [label_dict[lab] for lab in int_labels]
...
def main():
    ...
    conv_network.load_state_dict(torch.load(str(Path(__file__).parent.parent.parent) + '\\model_saves\\pneumonia_cnn.pt', map_location = DEVICE))
