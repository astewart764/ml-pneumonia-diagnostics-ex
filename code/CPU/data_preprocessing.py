...
def DatasetCreation(dataset_type):
    image_directory = str(Path(__file__).parent.parent.parent) + "\\" + dataset_type + "\\"     # Define directroy containing train, test, val
...
def SaveDataset(image_array, label_array, category):
    npy_directory = str(Path(__file__).parent.parent.parent) + '\\npy_dataset\\'
...
def BatchSegmentation(dataset, batch_size):
    batch_directory = str(Path(__file__).parent.parent.parent) + '\\training_batches\\'
    Path(batch_directory).mkdir(parents = True, exist_ok = True)

    print("No. of batch files : " + str(math.floor(len(dataset) / batch_size)))
    print("Unused files : " + str(trainImages.shape[0] - (32 * 32 * (math.floor(len(dataset) / batch_size)))))
...
def GetFactorial(num):
    ...
    [print(idx) for idx in range(1,num) if num % idx == 0]
...
BatchSegmentation(zipped_data, 32 * 32)
