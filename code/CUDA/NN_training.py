...
def train(network, kwargs, epochs, device):
    ...
    Path(str(Path(__file__).parent.parent.parent) + '\\model_saves\\').mkdir(parents = True, exist_ok = True)

    for epoch in range(epochs):
        running_loss    =   0
        iter_counter    =   0

        for jdx, file in enumerate(Path(str(Path(__file__).parent.parent.parent) + '\\training_batches\\').glob('**/*.npy')):
        ...
        torch.save(network.state_dict(), str(Path(__file__).parent.parent.parent) + '\\model_saves\\pneumonia_cnn.pt')
